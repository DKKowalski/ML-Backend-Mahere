...
def get_letter_rect(k, contours):
    ...
    for i in range(len(contours)):
        ...
        if i == k:
            continue
        elif cv2.contourArea(cnt) < 100:
            continue

        x1,y1,w1,h1 = cv2.boundingRect(cnt)

        if abs(x1 + w1/2 - (x + w/2)) < 100:
            if y1 > y:
                h = abs(y - (y1 + h1))
                w = abs(x - (x1 + w1))
            else:
...
def image_segmentation(filepath):
    ...
    kernel_size = 201
    normalized_mean = 20
    bin_img = cv2.adaptiveThreshold(src_img, PIXEL_SET, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, kernel_size,
                                    normalized_mean)
